...
import threading
from deeprl_signal_control.envs.test_env import GymEnv
...
def train(args):
    ...
    if env.agent == 'ia2c':
        ...
        model = MA2C(env.n_s_ls, env.n_a_ls, env.n_w_ls, env.n_f_ls, total_step,
                     config['MODEL_CONFIG'], seed=seed)
        predicted_values = np.zeros((len(env.n_s_ls),))  # Initialize predicted values

    # Disable multi-threading for safe implementation
    ...
    for epoch in range(total_step):
        obs, actions, rewards, next_obs, done = env.reset(), [], [], [], []
        for i in range(len(env.n_s_ls)):
            obs_with_pred = np.concatenate((obs[i], [predicted_values[i]]))
            action, _ = model.forward([obs_with_pred], done[i], out_type='pv')
            actions.append(action)
        while not all(done):
            next_obs, rs, dones = env.step(actions)
            rewards.append(rs)
            done = dones
            for i in range(len(env.n_s_ls)):
                obs_with_pred = np.concatenate((next_obs[i], [predicted_values[i]]))
                action, _ = model.forward([obs_with_pred], done[i], out_type='pv')
                actions[i] = action
        rewards = np.array(rewards).transpose()
        rewards_sum = np.sum(rewards, axis=0)
        
        # Update the predicted values using your prediction method
        predicted_values = predicted_values
        # predicted_values = your_prediction_method(obs, rewards_sum)

        model.add_transition(obs, actions, rewards_sum, next_obs, done)
        if epoch >= 10:  # Start training after a few epochs
            trainer.backward()

        if global_counter.should_test():
            trainer.save(model_dir=dirs['model'], global_step=global_counter.cur_step)
            if in_test:
        ...
        if global_counter.should_log():
            plot_train(global_counter.log, dirs['log'])
