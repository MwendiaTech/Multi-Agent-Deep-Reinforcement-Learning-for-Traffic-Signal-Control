...
from ..lstm.prediction import p_real
...
class MA2C(IA2C):
    ...
    def forward(self, obs, done, out_type='pv'):
        if len(out_type) == 1:
            ...
            out1, out2 = [], []
        for i in range(self.n_agent):
            # Modify the observation by adding predicted values to the state
            obs_with_pred = np.concatenate((obs[i], [predicted_values[i]]))
            
            cur_out = self.policy_ls[i].forward(self.sess, obs_with_pred, done, out_type)
            if len(out_type) == 1:
                out.append(cur_out)
            else:
                out1.append(cur_out[0])
    ...
    def add_transition(self, obs, actions, rewards, next_obs, done):
        # Modify the observation by adding predicted values to the state
        obs_with_pred = np.concatenate((obs, [[predicted_values[i]] for i in range(self.n_agent)]))
        ...
        if self.reward_clip:
            rewards = np.clip(rewards, -self.reward_clip, self.reward_clip)
        for i in range(self.n_agent):
            self.trans_buffer_ls[i].add_transition(obs_with_pred[i], actions[i],
